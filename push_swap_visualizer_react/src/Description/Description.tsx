...
function drawStackInfo(ctx: CanvasRenderingContext2D, list: string[], type: string[], x: number, y: number) {
	ctx.font = '25px Arial';
	for (let i = 0; i < 5; i++) {
...
function drawStack(ctx: CanvasRenderingContext2D, arr: IStackInfo[], cur: string) {
...
function drawStackBox(ctx: CanvasRenderingContext2D, width: number, height: number, xPos: number, yPos: number, name: string) {
	ctx.beginPath();
	...
	return (
		<DescriptionContainer>
			<h2 onClick={() => setUpper(!upper)}>
				<Direction className={upper ? 'on' : ''}>{'>'}</Direction> What is Push Swap?
			</h2>
			{upper && (
				<p>
					"push_swap" is a task to sort an unsorted stack A to stack B using a given set of commands while minimizing the number
					of commands used.
				</p>
			)}
			{upper && (
				<CommandSelector>
					{data.map((info: ICmdInfo, index: number) => (
						<div key={info.name} className={cmd === info.name ? 'active' : ''} onClick={() => onClick(info.name, index)}>
							{info.name}
						</div>
					))}
				</CommandSelector>
			...
			<h2 onClick={() => setBelow(!below)}>
				<Direction className={below ? 'on' : ''}>{'>'}</Direction> How can I solve it?
			</h2>
			{below && (
				<CommandSelector>
			...
			{below && <video controls={true} src={'./' + solutions[solIndex].file} width={'100%'} height={'600px'} />}
		</DescriptionContainer>
	);
};
