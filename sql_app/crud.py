...
def create_record(db:Session, record : schemas.RecordCreate):
  db_record = models.Record(id=record.id, country=record.country, answer_count=record.answer_count)
  db.add(db_record)
  db.commit()
...
def creaet_log(db:Session, log: schemas.LogCreate):
  db_log = models.Log(id=log.id, country=log.country, repo=log.repo, answer_count=log.answer_count)
  db.add(db_log)
  db.commit()
  db.refresh(db_log)
  return db_log